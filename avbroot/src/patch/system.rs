...
fn find_zip_bounds(data: &[u8], eocd_offset: usize) -> Option<Range<usize>> {
    ...
    trace!(start, end, "Found zip bounds");
    ...
    for index in 0..zip_reader.len() {
        ...
        if !entry.name().ends_with(".x509.pem") {
            ...
            trace!(path = entry.name(), "Excluded due to invalid name");
            return None;
        }
    }
...
pub fn patch_system_image(
    ...
    trace!(num_chunks, "Parallel heuristics search for otacerts.zip");
