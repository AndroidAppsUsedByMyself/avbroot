...
fn load_ramdisk(
    ...
    trace!(num_entries = entries.len(), format = ?reader.format(), "Loaded ramdisk");
...
fn save_ramdisk(
    ...
    trace!(num_entries = entries.len(), format = ?format, "Wrote ramdisk");
...
impl MagiskRootPatcher {
    ...
    pub fn new(
        ...
        debug!(version, "Found Magisk version");
    ...
    fn get_version(path: &Path) -> Result<u32> {
        ...
        loop {
            ...
            if let Some(suffix) = line.trim_end().strip_prefix("MAGISK_VER_CODE=") {
                trace!(line, "Magisk version code line");
        ...
        for ramdisk in ramdisks {
            ...
            for index in 0..zip.len() {
                ...
                if !zip_entry.name().ends_with(".x509.pem") {
                    debug!(path = zip_entry.name(), "Skipping invalid entry path");
                    continue;
                }
...
impl BootImagePatch for OtaCertPatcher {
    ...
    fn patch(&self, boot_image: &mut BootImage, cancel_signal: &AtomicBool) -> Result<()> {
        ...
        trace!(size = new_zip.len(), "Generated new otacerts.zip");
...
pub fn load_boot_images<'a>(
    ...
    names
        ...
        .map(|name| {
            ...
            trace!(name, image_size, "Loaded boot image");
...
pub fn patch_boot_images<'a>(
        ...
        .map(|p| {
            ...
            p.find_targets(&images, cancel_signal).and_then(|targets| {
                if targets.is_empty() {
                    ...
                    debug!(targets = targets.as_value(), "Found patcher targets");
                    Ok(targets)
                }
            })
        })
    ...
    debug!(all_targets = all_targets.as_value());
